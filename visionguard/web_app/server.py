            ...
            with app.config["PROCESS_LOCK"]:
                ...
                for event in pipe.index_video_iter(video["_path"]):
                    if event.get("kind") == "preview":
                        ...
                        set_job_stage(job, "keyframes_selected", "running", processed=kept, total=total, message=f"{kept} unique frames retained so far.")
                        ...
                        set_job_stage(job, "keyframes_selected", "completed", processed=len(frames), total=len(frames), message="Every decoded non-duplicate frame was retained for indexing.")
                        removed_duplicates = int(meta.get("skipped_exact_duplicate_frames", 0))
