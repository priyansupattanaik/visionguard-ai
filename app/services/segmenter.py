...
from app.services.nvidia_verifier import NvidiaFrameVerifier
from app.utils.cache import resolve_device


class GroundedSegmenter:
    def __init__(self, sam="facebook/sam2.1-hiera-small", verifier_model=None, verifier=None, device=None):
        self.sam_name = os.getenv("SAM_MODEL") or sam
        self.dev = resolve_device(device)
        v_model = os.getenv("NVIDIA_VLM_MODEL") or verifier_model
        self.ver = verifier or NvidiaFrameVerifier(model=v_model)
        self.sp = None
        self.sm = None
