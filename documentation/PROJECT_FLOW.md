...
4. Every retained frame is written as source evidence, then processed by YOLO detection and BoT-SORT tracking. The pipeline stores the original frame number, decoder timestamp, boxes, detector confidences, classes, track IDs, color/appearance metadata, and exact-deduplication status.
5. SigLIP embeds every retained frame. Retained frames are grouped into `WIN_SEC` evidence segments for segment-level retrieval; this grouping does not remove frames.
...
flowchart TD
    ...
    G --> H[YOLO detect every retained frame]
    H --> I[BoT-SORT track objects]
