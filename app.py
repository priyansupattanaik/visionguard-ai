...
def find_query(q):
    ...
    all_clips = [x["raw_clip"] for x in seg]
    note = f"### {seg[0]['label']}\n\n{seg[0]['summary']}\n\nsegmentation loads when you open a match." if seg else ""
    return "matches ready", ans, rows, first, all_clips, gr.update(choices=choices, value=choices[:1]), gr.update(choices=choices, value=choices[0] if choices else None), [], note, q.strip(), seg, gr.update(visible=False, value=None), gr.update(visible=False, value=None), gr.update(visible=False, value=None)
